...
def load_pretrained_encoder(model, device, fold, seed):
    checkpoint_path = f"SSL_models_pt/SSL_model_lr6e4_OPP_fold{fold}_seed{seed}.pt"
...
######################################################################
    # recalibrate BN running stats to downstream training distribution
    model.encoder.train()
    with torch.no_grad():
...
######################################################################
    ...
    optimizer = torch.optim.AdamW(filter(lambda param: param.requires_grad, model.parameters()), lr = lr, weight_decay = 1e-4)
 #  ----------- TRAINING -----------
    model_name = f"SSL_models_pt/model_PTC_lr3e4_OPP_fold{args.fold}_seed{seed}.pt"
    epoch_history = []
    ...
    bad_epochs = 0
    prof_out = None
    with open(f"logs/model_PTC_lr3e4_OPP_fold{args.fold}.txt", "a") as log_file:
