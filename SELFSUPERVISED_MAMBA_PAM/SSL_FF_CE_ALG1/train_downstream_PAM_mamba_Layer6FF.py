...
for seed in [42, 58, 7, 128, 92]: 
    ...
    # pass over train data 
    model.encoder.train()
    with torch.no_grad():
        for x_batch, _ in train_loader:
            model.encoder(x_batch.to(device, non_blocking = True))

    ################## note ##################
    # partial fine-tuning: freeze the whole encoder, then unfreeze ONLY the last
    # Mamba block + last norm_f (keeping subject-invariant low/mid features intact)
    for param in model.encoder.parameters():
        param.requires_grad = False
    for param in model.encoder.backbone.layers[-1].parameters():
        param.requires_grad = True
    for param in model.encoder.backbone.norm_f.parameters():
        param.requires_grad = True
    ...
    ################## note ##################
    # encoder group only gets the trainable params (last block + norm_f) at lr 1e-5
    optimizer = torch.optim.AdamW([{"params": [p for p in model.encoder.parameters() if p.requires_grad], "lr": 1e-5},{"params": model.classifier.parameters(), "lr": 3e-4}], weight_decay = 1e-4)
 #  ----------- TRAINING -----------
    model_name = f"SSL_models_pt/model_PTC_lr3e4_PAM_fold{args.fold}_seed{seed}.pt"
    epoch_history = []
    ...
    bad_epochs = 0
    prof_out = None
    with open(f"logs/model_PTC_lr3e4_PAM_fold{args.fold}.txt", "a") as log_file:
        ...
        for epoch in range(num_epochs):
            ...
            model.train()
            ################## note ##################
            # frozen front-end BN stays in eval: uses the recalibrated running stats
            # and does not update them 
            model.encoder.batch_layer.eval()
            total_loss = 0.0
            total_correct = 0
            total_samples = 0
