...
def validate_model_PRETRAIN(model, val_loader, device, criterion):
    ...
    for x_batch, _ in val_loader:
        ...
        #mask, x_batch_masked = masking_span_algorithm(x_batch)
...
for seed in [42, 58, 7, 128, 92]: # [42, 58, 7, 128, 92]
    ...
    #  ----------- TRAINING -----------
    model_name = f"SSL_models_pt/SSL_model_lr6e4_PAM_fold{args.fold}_seed{seed}.pt"
    epoch_history = []
    ...
    bad_epochs = 0
    prof_out = None
    with open(f"logs/SSL_training_lr6e4_PAM_fold{args.fold}.txt", "a") as log_file:
        ...
        for epoch in range(num_epochs):
            ...
            for _, (x_batch, _) in enumerate(loop):
                ...
                x_original = x_batch.clone()
                mask, x_batch_masked = masking_span_algorithm(x_batch, 0.15, 5)
                #mask, x_batch_masked = masking_span_algorithm(x_batch)
