...
class MambaClassificationModel(nn.Module):
    ...
    def save_pretrained(self, save_directory):
        ...
        '''
        os.makedirs(save_directory, exist_ok=True)  # create directory or if it's created it do not crash it. (exist_ok)
        
        # Model's state_dict, bin for weights
        model_path = os.path.join(save_directory, 'HAR_Mamba_model.bin')
        torch.save(self.state_dict(), model_path)
        
        # Model's configuration, JSON for config
        config_path = os.path.join(save_directory, 'HAR_Mamba_config.json')
